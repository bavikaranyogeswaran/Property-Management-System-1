...
export function TreasurerAssignmentDialog({ open, onOpenChange, treasurer, properties }: TreasurerAssignmentDialogProps) {
    ...
    const fetchAssignments = async () => {
        ...
        setIsLoading(true);
        try {
            const response = await apiClient.get(`/users/${treasurer.id}/assigned-properties`);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const response = await apiClient.get(`/users/${treasurer.id}/assigned-properties`);
            const serverAssignedIds: number[] = response.data.map((p: any) => p.property_id);

            const toAdd = assignedPropertyIds.filter(id => !serverAssignedIds.includes(id));
            const toRemove = serverAssignedIds.filter((id: number) => !assignedPropertyIds.includes(id));

            const promises = [
                ...toAdd.map((id: number) => apiClient.post('/users/assign-property', { userId: treasurer.id, propertyId: id })),
                ...toRemove.map((id: number) => apiClient.delete(`/users/${treasurer.id}/assign-property/${id}`))
            ];
            ...
            <DialogContent className="max-w-md">
                ...
                <div className="py-4 max-h-[60vh] overflow-y-auto space-y-2">
                    {isLoading ? (
                        ...
                        <p className="text-center text-gray-500 py-4">No active properties found.</p>
                    ) : (
                        properties.map(property => (
                            <div key={property.id} className="flex items-start space-x-3 p-3 rounded-md hover:bg-gray-50 border border-transparent hover:border-gray-200 transition-colors">
                                <Checkbox
                                    id={`prop-${property.id}`}
                                    checked={assignedPropertyIds.includes(Number(property.id))}
                                    onCheckedChange={(checked) => handleToggle(Number(property.id), checked as boolean)}
                                />
                                <div className="grid gap-1.5 leading-none">
                                    <Label htmlFor={`prop-${property.id}`} className="font-medium cursor-pointer">
                                        {property.name}
                                    </Label>
