    ...
    <Card className="col-span-full lg:col-span-2 shadow-sm border-slate-200">
      <CardHeader className="flex flex-row items-start justify-between space-y-0">
        <div>
          ...
          <CardDescription>
            Revenue vs Maintenance Expenses (Last 12 Months)
          </CardDescription>
        </div>
        <div className="text-right hidden sm:block">
          ...
          <p
            className={`text-lg font-bold ${netProfit >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}
          >
            {netProfit >= 0 ? '+' : ''}
            {formatLKR(netProfit)}
          </p>
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <YAxis
                ...
                tickFormatter={(val) =>
                  val > 1000 ? `LKR ${(val / 1000).toFixed(0)}k` : `LKR ${val}`
                }
              />
              <Tooltip
                cursor={{ fill: '#f8fafc' }}
