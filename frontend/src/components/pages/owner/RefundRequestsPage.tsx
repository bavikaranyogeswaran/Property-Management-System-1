...
export function RefundRequestsPage() {
  const { user } = useAuth();
  const { leases, approveRefund, disputeRefund, recordDisbursement } = useLease();
  const { properties, units } = useProperty();
  const { tenants } = useUser();

  const [selectedLeaseId, setSelectedLeaseId] = useState<string | null>(null);
  const [actionType, setActionType] = useState<'approve' | 'dispute' | 'disburse' | null>(null);
  ...
  const requests = leases.filter(l => 
    ['awaiting_approval', 'disputed', 'awaiting_acknowledgment'].includes(l.depositStatus || '')
  );
                  ...
                  return (
                    <TableRow key={lease.id}>
                      ...
                      <TableCell>
                        <Badge
                          variant={
                            lease.depositStatus === 'awaiting_approval' ? 'secondary' : 
                            lease.depositStatus === 'awaiting_acknowledgment' ? 'outline' : 'destructive'
                          }
                          className={
                            lease.depositStatus === 'awaiting_approval' ? 'bg-blue-100 text-blue-700' : 
                            lease.depositStatus === 'awaiting_acknowledgment' ? 'bg-orange-100 text-orange-700 border-orange-200' : 
                            'bg-red-100 text-red-700'
                          }
                        >
                          {lease.depositStatus === 'awaiting_approval' ? 'Awaiting Approval' : 
                           lease.depositStatus === 'awaiting_acknowledgment' ? 'Awaiting Disbursement' : 'Disputed'}
                        </Badge>
                      </TableCell>
                      <TableCell>
          ...
          <DialogHeader>
            <DialogTitle>
              {actionType === 'approve' ? 'Approve Refund' : 
               actionType === 'disburse' ? 'Record Bank Disbursement' : 'Dispute Refund'}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleAction} className="space-y-4 pt-4">
            {actionType === 'approve' ? (
              <div className="bg-blue-50 text-blue-800 p-4 rounded-lg text-sm">
                <p>Approve this refund request. This will move it to the <strong>Awaiting Disbursement</strong> state where the Treasurer can record the bank transfer.</p>
                <p className="mt-2 font-semibold">Amount: {formatLKR(leases.find(l => l.id === selectedLeaseId)?.proposedRefundAmount || 0)}</p>
              </div>
            ) : actionType === 'disburse' ? (
              <div className="space-y-4">
                <div className="bg-orange-50 text-orange-800 p-4 rounded-lg text-sm">
                  <p>Provide the physical bank transfer details to finalize this refund and record the cash outflow in the ledger.</p>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="bankRef">Bank Reference ID / UTR (Required)</Label>
                  <Input 
                    id="bankRef"
                    required
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button 
                ...
                variant={actionType === 'dispute' ? 'destructive' : actionType === 'disburse' ? 'default' : 'default'}
                className={actionType === 'disburse' ? 'bg-orange-600 hover:bg-orange-700 text-white' : ''}
              >
                {isSubmitting ? 'Processing...' : (
                  actionType === 'approve' ? 'Confirm Approval' : 
                  actionType === 'disburse' ? 'Finalize Disbursement' : 'Submit Dispute'
                )}
              </Button>
            </div>
          </form>
