...
export function LeasesPage() {
  ...
  const handleAddAdjustment = async (date: string, rent: number, notes: string) => {
  ...
  const handleSubmitRenewalProposal = async (id: string, rent: number, date: string, notes: string) => {
    try {
  ...
  const handleRefundAction = async (id: string, amount: number, notes: string, type: 'request' | 'approve' | 'dispute') => {
    try {
      if (type === 'request') {
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="p-0">
          <Tabs defaultValue="active" className="w-full">
            ...
            <TabsContent value="active" className="m-0">
              <LeaseTable leases={activeLeases} emptyIcon={FileText} emptyMessage="No active leases" {...tableProps} />
            </TabsContent>

            <TabsContent value="expiring" className="m-0">
              <LeaseTable leases={expiringSoon} emptyIcon={Calendar} emptyMessage="No leases expiring soon" {...tableProps} />
            </TabsContent>

            <TabsContent value="expired" className="m-0">
              <LeaseTable leases={expiredLeases} emptyIcon={AlertCircle} emptyMessage="No expired leases" {...tableProps} />
            </TabsContent>

            <TabsContent value="ended" className="m-0">
              <LeaseTable leases={endedLeases} emptyIcon={FileText} emptyMessage="No ended leases" {...tableProps} />
            </TabsContent>

            <TabsContent value="draft" className="m-0">
              <LeaseTable leases={draftLeases} emptyIcon={FileText} emptyMessage="No draft leases" {...tableProps} />
            </TabsContent>
                  ...
                  <TableBody>
                    {renewalRequests.map((request) => (
                      <TableRow key={request.id}>
                        ...
                        <TableCell>{formatLKR(request.currentMonthlyRent)}</TableCell>
                        <TableCell>
                          {request.proposedMonthlyRent ? (
                            <div className="text-sm">
                              <div className="font-medium text-emerald-600">{formatLKR(request.proposedMonthlyRent)}</div>
                        ...
                        <TableCell>
                          <Badge variant="outline" className={request.status === 'pending' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}>
                            {request.status}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-right">
                          <div className="flex gap-2 justify-end">
                            {(request.status === 'pending' || request.status === 'negotiating') && (
                              <>
                                <Button size="sm" variant="outline" onClick={() => setSelectedRenewal(request)}>Negotiate</Button>
                                {request.proposedMonthlyRent && (
                                  <Button size="sm" onClick={() => approveLeaseRenewal(request.id)} className="bg-emerald-600">Approve</Button>
                                )}
                                <Button size="sm" variant="ghost" onClick={() => rejectRenewal(request.id)} className="text-red-600">Reject</Button>
                              </>
                            )}
                          </div>
                        </TableCell>
      ...
      {/* --- Specialized Dialogs --- */}
      <RefundDialog leaseId={refundLeaseId} type={refundType} onClose={() => setRefundLeaseId(null)} onSubmit={handleRefundAction} leases={leases} />
      <TerminationDialog leaseId={endLeaseId} onClose={() => setEndLeaseId(null)} onSubmit={confirmEndLease} leases={leases} />
      <AdjustmentDialog leaseId={adjustmentsLeaseId} adjustments={adjustments} isLoading={isLoadingAdjustments} onClose={() => setAdjustmentsLeaseId(null)} onSubmit={handleAddAdjustment} />
      <RenewalDialog request={selectedRenewal} leases={leases} onClose={() => setSelectedRenewal(null)} onSubmit={handleSubmitRenewalProposal} />
      <VerificationDialog leaseId={rejectionLeaseId} onClose={() => setRejectionLeaseId(null)} onSubmit={confirmRejectDocuments} />
            ...
            <div className="space-y-4 py-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 bg-gray-50 rounded border">
                  ...
                  <p className="font-medium">{selectedLease.tenantName || 'Unknown'}</p>
                </div>
                <div className="p-3 bg-gray-50 rounded border">
              ...
              <div className="p-4 border rounded-md bg-blue-50/30 flex items-center gap-3">
                 ...
                 <div>
                    <Label className="text-xs text-blue-700">Security Deposit Status</Label>
                    <p className="font-semibold text-blue-900 capitalize">{selectedLease.depositStatus?.replace('_', ' ')}</p>
                 </div>
              </div>
              {selectedLease.documentUrl && (
                <div className="flex justify-center p-4 border rounded bg-white">
                  <img src={selectedLease.documentUrl || "/placeholder.svg"} alt="Lease Agreement" className="max-h-[300px] object-contain shadow-sm" />
                </div>
              )}
              <div className="flex justify-end pt-4">
                <Button variant="outline" onClick={() => setSelectedLease(null)}>Close</Button>
              </div>
            </div>
          )}
        </DialogContent>
      ...
      <Dialog open={!!finalizeLeaseId} onOpenChange={(open) => !open && setFinalizeLeaseId(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Finalize Move-Out</DialogTitle></DialogHeader>
          <p>Are you sure you want to finalize the move-out for this lease? This will close the record.</p>
          <div className="flex justify-end gap-3 pt-4">
            <Button variant="ghost" onClick={() => setFinalizeLeaseId(null)}>Cancel</Button>
      ...
      <Dialog open={!!markAvailableUnitId} onOpenChange={(open) => !open && setMarkAvailableUnitId(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Mark Unit Available</DialogTitle></DialogHeader>
          <p>Are you sure the unit is ready for new tenants?</p>
          <div className="flex justify-end gap-3 pt-4">
            <Button variant="ghost" onClick={() => setMarkAvailableUnitId(null)}>Cancel</Button>
            <Button onClick={confirmMarkUnitAvailable} className="bg-emerald-600">Mark Available</Button>
          </div>
        </DialogContent>
      </Dialog>

      <Dialog open={!!activateLeaseId} onOpenChange={(open) => !open && setActivateLeaseId(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Activate Lease</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <p>Ready to activate the lease? Deposit status: <Badge variant="outline">{depositStatus?.status || 'checking...'}</Badge></p>
            <div className="flex justify-end gap-3 pt-4">
              <Button variant="ghost" onClick={() => setActivateLeaseId(null)}>Cancel</Button>
              <Button onClick={confirmActivateLease} className="bg-green-600">Confirm Activation</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
