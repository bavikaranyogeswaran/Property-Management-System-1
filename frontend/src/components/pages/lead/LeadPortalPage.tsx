...
export function LeadPortalPage() {
    ...
    const loadData = async () => {
      try {
        ...
        setPortalData({
          ...
          activeLease: portalRes.data.activeLease
            ? {
                ...
                monthlyRent: toLKRFromCents(
                  portalRes.data.activeLease.monthlyRent
                ),
                currentDepositBalance: toLKRFromCents(
                  portalRes.data.activeLease.currentDepositBalance || 0
                ),
                targetDeposit: toLKRFromCents(
                  portalRes.data.activeLease.targetDeposit || 0
                ),
                depositStatus: portalRes.data.activeLease.depositStatus
                  ? {
                      ...portalRes.data.activeLease.depositStatus,
                      paidAmount: toLKRFromCents(
                        portalRes.data.activeLease.depositStatus.paidAmount
                      ),
                      targetAmount: toLKRFromCents(
                        portalRes.data.activeLease.targetAmount
                      ),
                    }
                  : null,
              }
        ...
        if (portalRes.data.lead) {
          setEditData({
            moveInDate: portalRes.data.lead.moveInDate
              ? portalRes.data.lead.moveInDate.split('T')[0]
              : '',
  ...
  const handleWithdraw = async () => {
    ...
    if (
      !confirm(
        'Are you sure you want to withdraw your application? This will release the unit and cancel your request.'
      )
    ) {
      return;
    }
        ...
        <Card className="max-w-md w-full">
          <CardContent className="pt-6 text-center space-y-4">
            ...
            <p className="text-sm text-gray-400">
              If you believe this is a mistake, please contact the property
              owner for a new access link.
            </p>
          </CardContent>
        </Card>
      </div>
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Your Application</h1>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              <div className="space-y-1 flex-1">
                <h3 className="text-lg font-semibold text-blue-900 flex items-center gap-2">
                  ...
                  <Badge className="bg-blue-200 text-blue-800 hover:bg-blue-200 border-0">
                    Draft Lease
                  </Badge>
                </h3>
                <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-blue-700 mt-2">
                  <div className="flex items-center gap-2">
                    <div
                      className={`size-2 rounded-full ${portalData.activeLease.depositStatus.isFullyPaid ? 'bg-green-500' : 'bg-yellow-500'}`}
                    />
                    Deposit:{' '}
                    {portalData.activeLease.depositStatus.isFullyPaid
                      ? 'Paid'
                      : `Pending (${formatLKR(portalData.activeLease.depositStatus.targetAmount - portalData.activeLease.depositStatus.paidAmount)} left)`}
                  </div>
                  <div className="flex items-center gap-2">
                    <div
                      className={`size-2 rounded-full ${
                        portalData.activeLease.verificationStatus === 'verified'
                          ? 'bg-green-500'
                          : portalData.activeLease.verificationStatus ===
                              'rejected'
                            ? 'bg-red-500'
                            : 'bg-yellow-500'
                      }`}
                  ...
                  <div className="mt-4 p-3 bg-red-50 border border-red-100 rounded-md flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-red-700">
                        {portalData.activeLease.verificationRejectionReason ||
                          'Please re-upload your identity documents.'}
                      </p>
                    </div>
                  </div>
                )}
                ...
                {!portalData.activeLease.depositStatus.isFullyPaid && (
                  <p className="text-sm text-blue-600 mt-2">
                    Please complete your deposit payment using the link sent to
                    your email to proceed with verification.
                  </p>
                )}
                {portalData.activeLease.depositStatus.isFullyPaid &&
                  portalData.activeLease.verificationStatus === 'pending' && (
                    <p className="text-sm text-blue-600 mt-2">
                      Payment received! Our staff is currently reviewing your
                      documents.
                    </p>
                  )}
              </div>
                ...
                {isEditingPrefs ? (
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          const val = e.target.value
                            ...
                            : undefined;
                          const term = portalData.leaseTerms.find(
                            (t) => t.leaseTermId === val
                          );
                          setEditData({
                            ...editData,
                      ...
                      >
                        ...
                        {portalData.leaseTerms.map((term) => (
                          <option
                            ...
                            value={term.leaseTermId}
                          >
                            {term.name} (
                            {term.type === 'fixed'
                              ? `${term.durationMonths} mo`
                              : 'Periodic'}
                            )
                          </option>
                        ))}
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                            setEditData({
                              ...editData,
                              preferredTermMonths: parseInt(e.target.value),
                            })
                          }
                          className="h-8 mt-1"
                        />
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-gray-900">
                        {lead.leaseTermId
                          ? portalData.leaseTerms.find(
                              (t) => t.leaseTermId === lead.leaseTermId
                            )?.name
