...
export function TenantLeasePage() {
  ...
  const getLeaseProgress = (lease: Lease) => {
    ...
    if (now <= start) return { progress: 0, daysRemaining: Math.ceil((end - start) / 86400000), totalDays: Math.ceil((end - start) / 86400000) };
    if (now >= end) return { progress: 100, daysRemaining: 0, totalDays: Math.ceil((end - start) / 86400000) };
                  ...
                  {daysRemaining <= 30 && daysRemaining > 0 && (
                    <div className="flex items-center gap-2 p-3 bg-red-50 rounded-lg border border-red-200">
                      ...
                      <p className="text-sm text-red-700">
                        Your lease is expiring soon. Contact your property owner about renewal.
                      </p>
                    </div>
                  )}
                </div>
