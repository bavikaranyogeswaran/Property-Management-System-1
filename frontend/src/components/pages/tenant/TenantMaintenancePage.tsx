                ...
                <div className="grid grid-cols-4 gap-2 mb-2">
                  {selectedImages.map((file, index) => (
                    <div key={index} className="relative aspect-square rounded-md overflow-hidden bg-gray-100 border">
                      ...
                      >
                        <AlertCircle className="size-3 rotate-45" /> {/* Using AlertCircle rotated as X substitute if X not imported, wait, let me check imports */}
                      </button>
                    </div>
                  ))}
                  <label
                      ...
                      {request.images && request.images.length > 0 && (
                        <div className="flex gap-2 mb-3 overflow-x-auto pb-2">
                          {request.images.map((img, idx) => (
                            <div key={idx} className="relative size-16 rounded-md overflow-hidden bg-gray-100 border flex-shrink-0">
                              <img
                                src={img}
                                alt="Attachment"
                                className="w-full h-full object-cover"
