...
export function OnboardingStatusPage() {
  ...
  const fetchStatus = async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.error || 'Failed to load onboarding status.'
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleReupload = async () => {
    ...
    try {
      ...
      toast.success(
        'Document re-uploaded successfully! Our team will review it.'
      );
      setFile(null);
      fetchStatus();
    } catch (err: any) {
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <Card className="max-w-md w-full">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              {error || 'The link you used is invalid or has expired.'}
            </CardDescription>
          </CardHeader>
          <CardContent className="text-center space-y-4">
            <p className="text-sm text-gray-500">
              If you believe this is an error, please contact the property
              manager or check your email for a new link.
            </p>
      ...
      <div className="max-w-3xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-lg text-gray-600 font-medium">
            Lease for{' '}
            <span className="text-primary">{status.property.name}</span> • Unit{' '}
            {status.property.unitNumber}
          </p>
        </div>
        ...
        {/* Phase 1: Payment */}
        <Card
          className={`border-l-4 ${isDepositPaid ? 'border-l-green-500' : 'border-l-blue-500'} shadow-sm`}
        >
          <CardHeader className="pb-4">
            <div className="flex justify-between items-start">
              <div className="flex gap-4">
                <div
                  className={`p-2 rounded-xl ${isDepositPaid ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}
                >
                ...
                <div>
                  ...
                  <CardDescription className="text-base">
                    Holding deposit to secure your unit:{' '}
                    <span className="font-bold text-gray-900">
                      {formatLKR(toLKRFromCents(status.invoice.amount))}
                    </span>
                  </CardDescription>
                </div>
              </div>
            ...
            <CardContent className="pt-0">
              <Alert className="bg-amber-50 border-amber-200">
                ...
                <AlertDescription className="text-amber-800 font-medium">
                  Please complete the deposit payment to reserve this property
                  and proceed to document verification.
                </AlertDescription>
              </Alert>
              <Button
                asChild
                className="w-full mt-4 h-12 text-lg font-bold shadow-lg shadow-primary/20 transition-all hover:scale-[1.01]"
              >
                <Link to={`/pay/${token}`}>
                  Pay Deposit Now <ArrowRight className="ml-2 h-5 w-5" />
                </Link>
              </Button>
            </CardContent>
          )}
        ...
        <Card
          className={`border-l-4 ${isVerified ? 'border-l-green-500' : isRejected ? 'border-l-red-500' : 'border-l-blue-500'} shadow-sm`}
        >
          <CardHeader className="pb-4">
            <div className="flex justify-between items-start">
              <div className="flex gap-4">
                <div
                  className={`p-2 rounded-xl ${isVerified ? 'bg-green-100 text-green-700' : isRejected ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}`}
                >
                ...
                <div>
                  ...
                  <CardDescription className="text-base">
                    Identity and proof of income documents (NIC, Payslips)
                  </CardDescription>
                </div>
              </div>
              {isVerified ? (
          ...
          <CardContent className="space-y-4">
            {isRejected && (
              <Alert
                variant="destructive"
                className="bg-red-50 border-red-200 text-red-900 border-l-4"
              >
                ...
                <AlertDescription className="font-medium mt-1">
                  ...
                  <span className="font-black italic underline">
                    {status.lease.verification.reason ||
                      'Documents provided were unclear or invalid.'}
                  </span>
                </AlertDescription>
              </Alert>
            )}
              ...
              <div className="space-y-4">
                <div className="grid w-full items-center gap-1.5">
                  ...
                  <p className="text-xs text-sidebar-foreground-muted font-medium italic">
                    Combine your NIC front/back and latest payslip into a single
                    PDF or Image.
                  </p>
                </div>
              </div>
            ...
            {isDocUploaded && !isRejected && !isVerified && (
              <Alert className="bg-blue-50 border-blue-200 border-l-4">
                ...
                <AlertDescription className="text-blue-800 font-medium leading-relaxed">
                  Our team has received your documents and is currently
                  reviewing your identity and income details. This usually takes
                  **1-2 business days**. We will notify you via email once
                  complete.
                </AlertDescription>
              </Alert>
            )}
        ...
        {/* Phase 3: Signing */}
        <Card
          className={`border-l-4 ${isVerified ? 'border-l-blue-500' : 'border-l-gray-300 opacity-60'} shadow-sm transition-all grayscale-[0.5] hover:grayscale-0`}
        >
          <CardHeader>
            <div className="flex justify-between items-start">
              <div className="flex gap-4">
                <div
                  className={`p-2 rounded-xl ${isVerified ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'}`}
                >
                ...
                <div>
                  ...
                  <CardDescription className="text-base text-gray-500">
                    Official digital signing and key handover schedule
                  </CardDescription>
                </div>
              </div>
            ...
            <CardContent>
              <Alert className="bg-green-50 border-green-200 border-l-4">
                ...
                <AlertDescription className="text-green-800 font-bold">
                  Verification Complete! Our property manager is preparing your
                  formal digital agreement. You will receive a final email to
                  set your portal password and sign the lease.
                </AlertDescription>
              </Alert>
            </CardContent>
          )}
        ...
        <p className="text-center text-xs text-sidebar-foreground-muted font-medium mt-8 pb-12">
          Secure onboarding provided by Antigravity Property Management. Link
          expires in 7 days.
        </p>
      </div>
    </div>
