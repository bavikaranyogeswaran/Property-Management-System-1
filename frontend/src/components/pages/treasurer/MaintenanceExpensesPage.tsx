...
import { Plus, DollarSign, Calendar, FileText, Wrench, Trash2 } from 'lucide-react';
import { toast } from 'sonner';

export function MaintenanceExpensesPage() {
    const { maintenanceRequests, maintenanceCosts, addMaintenanceCost, deleteMaintenanceCost, units, properties, tenants } = useApp();
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-2xl font-semibold text-gray-900">Maintenance Expenses</h2>
                    <p className="text-sm text-gray-500 mt-1">Track costs for maintenance requests</p>
                </div>
            </div>
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {requests.map((request) => {
                                    ...
                                    const requestCosts = maintenanceCosts.filter(c => c.requestId === request.id);

                                    return (
                                        <React.Fragment key={request.id}>
                                            <TableRow className={requestCosts.length > 0 ? "border-b-0" : ""}>
                                                ...
                                                <TableCell className="align-top">
                                                    <Badge variant={
                                                        ...
                                                        request.status === 'completed' ? 'bg-green-100 text-green-800 hover:bg-green-100 border-transparent shadow-none' :
                                                            request.status === 'in_progress' ? 'bg-blue-100 text-blue-800 hover:bg-blue-100 border-transparent shadow-none' :
                                                                'text-gray-600'
                                                    }>
                                                ...
                                                <TableCell className="align-top font-medium">
                                                    {totalCost > 0 ? `LKR ${totalCost.toLocaleString()}` : '-'}
                                                </TableCell>
                                                            ...
                                                            {requestCosts.map(cost => (
                                                                <div key={cost.id} className="flex justify-between items-center text-sm group">
                                                                    ...
                                                                    <div className="flex items-center gap-3">
                                                                        <span className="font-medium">LKR {cost.amount.toLocaleString()}</span>
                                                                        <button
                                                                            onClick={() => {
