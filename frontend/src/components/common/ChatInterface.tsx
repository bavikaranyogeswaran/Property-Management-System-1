                ...
                <div className="flex-1 p-4 overflow-y-auto" ref={scrollRef}>
                    <div className="space-y-4">
                        {messages.length === 0 ? (
                            ...
                            messages.map((msg) => {
                                const isMe = msg.senderId === Number(user?.id) || (msg.senderRole === user?.role && msg.senderName === user?.name);
                                ...
                                return (
                                    ...
                                    >
                                        <div
                                            className={`max-w-[80%] rounded-lg p-3 ${isSenderMe
                                        ...
                                        >
                                            {!isSenderMe && (
                                                <p className="text-xs font-semibold mb-1 opacity-70">
                                                    {msg.senderRole === 'owner' ? 'Owner' : msg.senderName || 'User'}
                                                </p>
                                            )}
                                            <p className="text-sm whitespace-pre-wrap">{msg.content}</p>
                                            <p className={`text-[10px] mt-1 text-right ${isSenderMe ? 'text-blue-100' : 'text-gray-500'}`}>
                ...
                {!readOnly && (
                    <div className="p-4 border-t bg-gray-50">
                        <form onSubmit={handleSendMessage} className="flex gap-2">
                            ...
                            />
                            <Button type="submit" disabled={isLoading || !newMessage.trim()} className="h-auto">
