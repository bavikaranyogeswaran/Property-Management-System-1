                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="space-y-1 mt-2">
                                            ...
                                            <div className="flex items-center gap-2 text-sm text-gray-600">
                                                ...
                                                Member since {tenant.createdAt ? new Date(tenant.createdAt).toLocaleDateString() : 'N/A'}
                                            </div>
                                        </div>
                                    </div>
                                    {lease ? (
                                        <Badge variant="secondary" className="bg-green-100 text-green-700">Active Tenant</Badge>
                                    ) : (
                                    ...
                                    {lease && unit && property ? (
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <p className="text-sm text-gray-500">{property.propertyNo} {property.street}, {property.city}</p>
                                            </div>
                                            <div>
                            ...
                            {allTenantLeases.length > 0 ? (
                                <div className="space-y-2">
                                    {allTenantLeases.map((l) => {
                                        ...
                                        const lProp = lUnit ? properties.find(p => p.id === lUnit.propertyId) : null;
                                        return (
                                            <div key={l.id} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                                                <div className="text-sm">
                                                    <span className="font-medium">{lProp?.name}</span> - Unit {lUnit?.unitNumber}
                                                </div>
