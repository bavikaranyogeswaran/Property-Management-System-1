  ...
  const stats = [
    {
      ...
      subtitle: `$${totalPendingAmount.toLocaleString()}`,
      icon: Clock,
    ...
    },
    {
      ...
      subtitle: `$${totalVerifiedAmount.toLocaleString()}`,
      icon: CheckCircle,
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex justify-between items-center p-3 bg-blue-50 rounded-lg">
                ...
                <span className="text-lg font-semibold">${expectedMonthlyRevenue.toLocaleString()}</span>
              </div>
              <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                <span className="text-sm text-gray-700">Collected</span>
                <span className="text-lg font-semibold text-green-700">${totalVerifiedAmount.toLocaleString()}</span>
              </div>
              <div className="flex justify-between items-center p-3 bg-orange-50 rounded-lg">
                <span className="text-sm text-gray-700">Pending Verification</span>
                <span className="text-lg font-semibold text-orange-700">${totalPendingAmount.toLocaleString()}</span>
              </div>
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span className="text-sm text-gray-700">Outstanding</span>
                <span className="text-lg font-semibold text-gray-700">
                  ${(expectedMonthlyRevenue - totalVerifiedAmount - totalPendingAmount).toLocaleString()}
                </span>
              </div>
            </div>
          </CardContent>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="p-4 bg-green-50 rounded-lg border border-green-200">
              ...
              <p className="text-xs text-gray-600 mt-1">
                ${paidInvoices.reduce((sum, i) => sum + i.amount, 0).toLocaleString()}
              </p>
            </div>
