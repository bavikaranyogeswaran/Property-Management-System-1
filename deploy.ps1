...
Write-Host "🚀 Starting PMS Deployment Orchestrator..." -ForegroundColor Cyan

# 1. Environment Validation
if (-not (Test-Path ".env")) {
    Write-Warning "⚠️  No .env file found! Please copy .env.example to .env and fill in the secrets."
    exit 1
}
...
foreach ($var in $requiredVars) {
    if (-not (Select-String -Path ".env" -Pattern "^$var=")) {
        Write-Error "❌ Missing required secret: $var in .env!"
        exit 1
    }
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Error "❌ Docker Desktop is not running or not in PATH!"
    exit 1
}

# 3. Pull & Build
Write-Host "🏗️  Building production assets (this may take 1-2 minutes)..." -ForegroundColor Yellow
docker compose build --pull

# 4. Launch Stack
Write-Host "🚢 Launching containers in detached mode..." -ForegroundColor Blue
docker compose up -d

# 5. Health Monitoring
Write-Host "⏳ Waiting for system healthchecks (Database & Redis)..." -ForegroundColor Yellow
$timeout = 60
$elapsed = 0
