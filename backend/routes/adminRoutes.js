...
router.post('/trigger-late-fees', authenticateToken, async (req, res) => {
    try {
        if (req.user.role !== 'owner' && req.user.role !== 'treasurer') {
            return res.status(403).json({ error: 'Access denied. Only Owners or Treasurers can trigger late fees.' });
        }

        console.log(`Manual Late Fee Triggered by ${req.user.name} (${req.user.role})`);
        
        // This is an async function in cronJobs.js
        await applyLateFees();

        res.json({ status: 'ok', message: 'Late fee automation completed successfully.' });
    } catch (error) {
