        ...
        if (rows.length === 0) {
            ...
            rows.forEach((row, index) => {
                if (!row.l_tenant_id) {
                    ...
                    if (!row.p_payment_id) console.log("  -> Missing Payment (r.payment_id matching p.payment_id)");
                    else if (!row.i_invoice_id) console.log(`  -> Missing Invoice (p.invoice_id: ${row.p_invoice_id})`);
                    else if (!row.l_lease_id) console.log(`  -> Missing Lease (i.lease_id: ${row.i_lease_id})`);
                    else console.log(`  -> Missing Tenant (l.tenant_id is NULL in lease ${row.l_lease_id})`);
                } else {
