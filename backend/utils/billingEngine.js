...
export const calculateMonthlyRent = (lease, year, month, adjustments = []) => {
    ...
    const monthAdjustments = adjustments
        ...
        .sort((a, b) => parseLocalDate(a.effective_date) - parseLocalDate(b.effective_date));
    ...
    if (monthAdjustments.length === 0) {
        ...
        if (leaseEnd && leaseEnd.getFullYear() === year && (leaseEnd.getMonth() + 1) === month) {
            ...
            const startLimit = (leaseStart > billingMonthStart) ? Math.min(leaseStart.getDate(), 30) : 1;
        ...
        let currentStartDay = (leaseStart > billingMonthStart) ? Math.min(leaseStart.getDate(), 30) : 1;
        ...
        for (const adj of monthAdjustments) {
            ...
            if (segmentEndDay >= currentStartDay) {
                ...
                const segmentRent = moneyMath(currentRate).div(FIXED_DAYS_IN_MONTH).mul(segmentDays).round().value();
        ...
        const monthEndDay = (leaseEnd && leaseEnd < billingMonthEnd) ? Math.min(leaseEnd.getDate(), 30) : 30;
        if (monthEndDay >= currentStartDay) {
            const finalDays = monthEndDay - currentStartDay + 1;
            const finalRent = moneyMath(currentRate).div(FIXED_DAYS_IN_MONTH).mul(finalDays).round().value();
