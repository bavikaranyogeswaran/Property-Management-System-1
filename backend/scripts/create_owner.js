...
async function createOwner() {
    try {
        ...
        const [existing] = await db.query("SELECT * FROM users WHERE email = ?", [ownerDetails.email]);

        let userId;

        if (existing.length > 0) {
            userId = existing[0].user_id;
            console.log(`User ${ownerDetails.email} already exists (ID: ${userId}). Proceeding to create/update Owner profile.`);
        } else {
            ...
            try {
                const [userResult] = await db.query(
                    ...
                    [ownerDetails.id, ownerDetails.name, ownerDetails.email, ownerDetails.phone, passwordHash]
                );
                ...
                if (err.code === 'ER_DUP_ENTRY') {
                    ...
                    const [userResultAuto] = await db.query(
                        `INSERT INTO users (name, email, phone, password_hash, role, status, is_email_verified) 
                         VALUES (?, ?, ?, ?, 'owner', 'active', 1)`,
                        [ownerDetails.name, ownerDetails.email, ownerDetails.phone, passwordHash]
                    );
        ...
        const [existingOwner] = await db.query("SELECT * FROM owners WHERE user_id = ?", [userId]);
        ...
        if (existingOwner.length > 0) {
            ...
            await db.query(
                ...
                [ownerDetails.nic, ownerDetails.tin, ownerDetails.bank_name, ownerDetails.branch_name, ownerDetails.account_holder_name, ownerDetails.account_number, userId]
            );
        } else {
            await db.query(
