...
class UnitTypeController {
    ...
    async createUnitType(req, res) {
        try {
            ...
            if (error.code === 'ER_DUP_ENTRY') {
                return res.status(400).json({ error: 'Unit type with this name already exists' });
            }
            res.status(500).json({ error: error.message });
        }
    }
    ...
    async updateUnitType(req, res) {
        try {
            ...
            const success = await unitTypeModel.update(req.params.id, { name, description });
            if (!success) {
                return res.status(404).json({ error: 'Unit type not found' });
            }
    ...
    async deleteUnitType(req, res) {
        try {
            ...
            if (error.code === 'ER_ROW_IS_REFERENCED_2') {
                return res.status(400).json({ error: 'Cannot delete unit type that is in use by units' });
            }
            res.status(500).json({ error: error.message });
        }
    }
