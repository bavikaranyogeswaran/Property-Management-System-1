...
class RenewalController {
    ...
    async proposeTerms(req, res) {
        try {
            ...
            if (!proposedMonthlyRent || !proposedEndDate) {
                return res.status(400).json({ error: 'Proposed rent and end date are required' });
            }

            await renewalService.proposeTerms(id, {
                proposedMonthlyRent,
                proposedEndDate,
                notes
            }, req.user);
    ...
    async approveRenewal(req, res) {
        try {
            ...
            res.json({ message: 'Renewal approved and draft lease created', ...result });
        } catch (error) {
            if (error.message.includes('not found')) return res.status(404).json({ error: error.message });
