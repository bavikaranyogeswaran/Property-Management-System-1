...
class PayoutController {
    ...
    async previewPayout(req, res) {
        try {
            ...
            if (req.user.role !== 'owner') {
                return res.status(403).json({ error: 'Only owners can generate their payouts' });
            }
            ...
            const calculation = await payoutModel.calculateNetPayout(ownerId, startDate, endDate);
    ...
    async createPayout(req, res) {
        try {
            ...
            res.status(201).json({ message: 'Payout record created', payoutId, netPayout });
        } catch (error) {
            console.error(error);
