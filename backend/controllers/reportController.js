...
function drawRecommendation(doc, y, text, urgency = 'info') {
  ...
  doc
    ...
    .font('Helvetica-Bold')
    .fontSize(10)
    .text(icons[urgency], 62, y + 5);
  doc
    .fillColor('#334155')
    .font('Helvetica')
    .fontSize(9)
    .text(text, 76, y + 6, { width: 460, lineGap: 3 });
  doc.restore();
  doc.fillColor('black').font('Helvetica').fontSize(10);
  return y + 50;
}
...
class ReportController {
  ...
  async generateFinancialReport(req, res) {
    try {
      ...
      const propertyStats = await reportService.getFinancialStats(
        year,
        req.user
      );
      ...
      const propertyMetrics = entries.map(([name, stats]) => {
        ...
        const margin =
          stats.income > 0 ? ((net / stats.income) * 100).toFixed(1) : '0.0';
      ...
      drawKpiBox(
        doc,
        50 + (kpiWidth + spacing) * 2,
        y,
        kpiWidth,
        'NET INCOME',
        `LKR ${totalNet.toLocaleString()}`,
        totalNet >= 0 ? '#2563eb' : '#ef4444'
      );
      drawKpiBox(
        doc,
        50 + (kpiWidth + spacing) * 3,
        y,
        kpiWidth,
        'PROFIT MARGIN',
        `${totalMargin}%`,
        Number(totalMargin) >= 20 ? '#22c55e' : '#f59e0b'
      );
      y += 70;
      ...
      for (const p of propertyMetrics) {
        ...
        doc.text(`LKR ${p.income.toLocaleString()}`, 200, y, {
          width: 85,
          align: 'right',
        });
        doc.text(`LKR ${p.expense.toLocaleString()}`, 285, y, {
          width: 85,
          align: 'right',
        });
        doc
          .fillColor(isLoss ? '#ef4444' : '#22c55e')
          .text(`LKR ${p.net.toLocaleString()}`, 370, y, {
            width: 110,
            align: 'right',
          });
        doc
          ...
          .text(`${p.margin}%`, 485, y, { width: 65, align: 'right' });
        doc.fillColor('black');
        y += 18;
      }
      ...
      doc.text('TOTAL', 50, y);
      doc.text(`LKR ${totalIncome.toLocaleString()}`, 200, y, {
        width: 85,
        align: 'right',
      });
      doc.text(`LKR ${totalExpense.toLocaleString()}`, 285, y, {
        width: 85,
        align: 'right',
      });
      doc
        .fillColor(totalNet >= 0 ? '#22c55e' : '#ef4444')
        .text(`LKR ${totalNet.toLocaleString()}`, 370, y, {
          width: 110,
          align: 'right',
        });
      doc
        .fillColor('#1e293b')
        .text(`${totalMargin}%`, 485, y, { width: 65, align: 'right' });
      doc.fillColor('black');
      y += 30;
      ...
      if (entries.length === 0) {
        ...
        if (lossProperties.length > 0) {
          y = drawRecommendation(
            doc,
            y,
            `${lossProperties.length} propert${lossProperties.length > 1 ? 'ies are' : 'y is'} running at a loss: ${lossProperties.map((p) => p.name).join(', ')}. Review expense allocation and consider rent adjustments or cost-cutting measures.`,
            'critical'
          );
        } else {
          y = drawRecommendation(
            doc,
            y,
            `All properties are profitable. Best performer: "${best.name}" at ${best.margin}% margin.`,
            'success'
          );
        }
        ...
        if (propertyMetrics.length > 1 && best.name !== worst.name) {
          ...
          y = drawRecommendation(
            doc,
            y,
            `Lowest margin property: "${worst.name}" at ${worst.margin}%. Consider investigating operational costs or increasing occupancy to improve returns.`,
            worst.margin < 10 ? 'warning' : 'info'
          );
        }
        ...
        if (Number(totalMargin) < 15) {
          ...
          y = drawRecommendation(
            doc,
            y,
            `Overall profit margin (${totalMargin}%) is below the 15% healthy threshold. Evaluate portfolio-wide expense reduction strategies.`,
            'warning'
          );
        }
      }
  ...
  async generateOccupancyReport(req, res) {
    try {
      ...
      res.setHeader(
        'Content-Disposition',
        `inline; filename=occupancy_report.pdf`
      );
      doc.pipe(res);
      ...
      for (const p of propertyMetrics) {
        ...
        const rateColor =
          p.rate >= 90 ? '#22c55e' : p.rate >= 70 ? '#f59e0b' : '#ef4444';
        const urgency =
          p.rate >= 90
            ? 'Healthy'
            : p.rate >= 70
        ...
        if (p.vacancies.length > 0) {
          doc
            .fillColor('#ef4444')
            .text(`Vacant units: ${p.vacancies.join(', ')}`, 50, y);
          y += 14;
        }
        doc.fillColor('black');
        doc
      ...
      if (entries.length === 0) {
        y = drawRecommendation(
          doc,
          y,
          'No property data available. Ensure properties and units are registered in the system.',
          'warning'
        );
        ...
        y = drawRecommendation(
          doc,
          y,
          'Excellent — all units across your portfolio are fully occupied. Consider evaluating rent pricing for the next renewal cycle.',
          'success'
        );
      } else {
        const worstProperty = propertyMetrics.reduce((a, b) =>
          a.rate < b.rate ? a : b
        );
        y = drawRecommendation(
          doc,
          y,
          `${totalVacant} vacant unit${totalVacant > 1 ? 's' : ''} across your portfolio. Prioritize filling vacancies at "${worstProperty.name}" (${worstProperty.rate}% occupancy). Each month of vacancy is lost rental income.`,
          'critical'
        );
        ...
        if (lowProperties.length > 0) {
          ...
          y = drawRecommendation(
            doc,
            y,
            `${lowProperties.length} propert${lowProperties.length > 1 ? 'ies have' : 'y has'} occupancy below 70%. Consider adjusting pricing, improving marketing, or reviewing unit conditions.`,
            'warning'
          );
        }
      }
  ...
  async generateTenantRiskReport(req, res) {
    try {
      ...
      res.setHeader(
        'Content-Disposition',
        `inline; filename=tenant_risk_report.pdf`
      );
      doc.pipe(res);
      ...
      const avgScore =
        tenants.length > 0
          ? Math.round(
              tenants.reduce((s, t) => s + t.behavior_score, 0) / tenants.length
            )
          : 0;
      ...
      drawKpiBox(
        ...
        'HIGH RISK',
        `${highRisk.length}`,
        highRisk.length > 0 ? '#ef4444' : '#22c55e'
      );
      drawKpiBox(
        doc,
        50 + (kpiWidth + spacing) * 2,
        ...
        'MEDIUM RISK',
        `${medRisk.length}`,
        medRisk.length > 0 ? '#f59e0b' : '#22c55e'
      );
      drawKpiBox(
        doc,
        50 + (kpiWidth + spacing) * 3,
      ...
      for (const tenant of tenants) {
        ...
        doc.text(tenant.behavior_score.toString(), 220, y, {
          width: 50,
          align: 'center',
        });
        doc
          .fillColor(tenant.overdue_count > 0 ? '#ef4444' : '#334155')
          .text(tenant.overdue_count.toString(), 280, y, {
            width: 50,
            align: 'center',
          });
        doc.fillColor('#334155').text(tenant.paid_count.toString(), 340, y, {
          width: 50,
          align: 'center',
        });
        doc
          ...
          .font('Helvetica-Bold')
          .text(tenant.riskLevel, 410, y, { width: 70, align: 'center' });
        doc
          .fillColor(actionColor)
          .font('Helvetica-Bold')
          .text(action, 485, y, { width: 70, align: 'center' });
      ...
      if (tenants.length === 0) {
        y = drawRecommendation(
          doc,
          y,
          'No active tenants found. Ensure leases are active and tenants are registered.',
          'warning'
        );
        ...
        y = drawRecommendation(
          doc,
          y,
          `${highRisk.length} tenant${highRisk.length > 1 ? 's require' : ' requires'} immediate attention: ${highRisk.map((t) => t.name).join(', ')}. Schedule meetings to discuss payment plans. Consider issuing formal warnings for tenants with 3+ overdue invoices.`,
          'critical'
        );
        ...
        y = drawRecommendation(
          doc,
          y,
          `${medRisk.length} tenant${medRisk.length > 1 ? 's are' : ' is'} at medium risk. Proactively monitor payment patterns and schedule check-ins before they escalate.`,
          'warning'
        );
      } else {
        y = drawRecommendation(
          doc,
          y,
          `All ${tenants.length} tenants are in good standing. Average behavior score: ${avgScore}/100. Portfolio risk is minimal.`,
          'success'
        );
      }
      ...
      if (avgScore < 60 && tenants.length > 0) {
        ...
        y = drawRecommendation(
          doc,
          y,
          `Portfolio average behavior score (${avgScore}/100) is below healthy levels. Review tenant screening criteria for future leases.`,
          'warning'
        );
      }
  ...
  async generateMaintenanceCategoryReport(req, res) {
    try {
      const { categories, totalCost } =
        await reportService.getMaintenanceCategoryStats(req.user);
      const doc = new PDFDocument({ margin: 50 });
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader(
        'Content-Disposition',
        `inline; filename=maintenance_category_report.pdf`
      );
      doc.pipe(res);
      ...
      drawKpiBox(
        ...
        kpiWidth,
        'TOTAL SPEND',
        totalCost > 0 ? `LKR ${totalCost.toLocaleString()}` : 'LKR 0',
        totalCost > 0 ? '#ef4444' : '#22c55e'
      );
      drawKpiBox(
        doc,
        50 + kpiWidth + spacing,
      ...
      drawKpiBox(
        ...
        'TOP CATEGORY %',
        `${topPct}%`,
        Number(topPct) > 50 ? '#ef4444' : '#22c55e'
      );
      y += 70;
      ...
      if (totalCost === 0) {
        ...
        y = drawRecommendation(
          doc,
          y,
          'No maintenance costs have been recorded. This report will populate as maintenance expenses are logged.',
          'info'
        );
      } else {
        ...
        sorted.forEach(([cat, amount], index) => {
          ...
          doc.text(amount.toLocaleString(), 200, y, {
            width: 90,
            align: 'right',
          });
        ...
        if (Number(topPct) > 50) {
          y = drawRecommendation(
            doc,
            y,
            `"${topCategory[0]}" accounts for ${topPct}% of total maintenance spend (LKR ${topCategory[1].toLocaleString()}). This concentration suggests a systemic issue — consider preventive maintenance or vendor renegotiation for this category.`,
            'critical'
          );
          ...
          y = drawRecommendation(
            doc,
            y,
            `"${topCategory[0]}" is the largest expense at ${topPct}%. Monitor this category quarterly for trends and consider bulk service contracts.`,
            'warning'
          );
        } else {
          y = drawRecommendation(
            doc,
            y,
            `Maintenance costs are well-distributed across ${categoryCount} categories. No single category dominates spending. Continue routine preventive maintenance.`,
            'success'
          );
        }
        ...
        if (categoryCount === 1) {
          ...
          y = drawRecommendation(
            doc,
            y,
            'Only one maintenance category detected. Ensure cost descriptions are detailed enough for accurate auto-categorization.',
            'info'
          );
        }
      }
  ...
  async generateLeaseExpirationReport(req, res) {
    try {
      ...
      res.setHeader(
        'Content-Disposition',
        `inline; filename=lease_expiration_forecast.pdf`
      );
      doc.pipe(res);
      ...
      drawReportHeader(
        ...
        'Upcoming lease renewals, revenue at risk, and urgency-based action plan (next 90 days)'
      );
      doc.moveDown(0.5);
      ...
      drawKpiBox(
        ...
        kpiWidth,
        'EXPIRING LEASES',
        `${leasesWithDays.length}`,
        leasesWithDays.length > 0 ? '#f59e0b' : '#22c55e'
      );
      drawKpiBox(
        doc,
        50 + kpiWidth + spacing,
        ...
        'CRITICAL (≤14d)',
        `${critical.length}`,
        critical.length > 0 ? '#ef4444' : '#22c55e'
      );
      drawKpiBox(
        doc,
        50 + (kpiWidth + spacing) * 2,
      ...
      drawKpiBox(
        ...
        kpiWidth,
        'REVENUE AT RISK',
        `LKR ${revenueAtRisk.toLocaleString()}/mo`,
        revenueAtRisk > 0 ? '#ef4444' : '#22c55e'
      );
      y += 70;
      ...
      if (leasesWithDays.length === 0) {
        ...
        y = drawRecommendation(
          doc,
          y,
          'No leases are expiring within the next 90 days. Portfolio lease stability is excellent.',
          'success'
        );
      } else {
        // --- Expiration Table ---
        ...
        for (const lease of leasesWithDays) {
          ...
          const urgencyLabel =
            lease.urgency === 'critical'
              ? 'CRITICAL'
              : lease.urgency === 'urgent'
                ? 'URGENT'
                : 'Upcoming';

          doc
            .fillColor('#334155')
            .text(lease.propertyName || 'N/A', 50, y, { width: 135 });
          doc.text(lease.unitNumber || 'N/A', 190, y);
          doc.text(lease.endDate, 245, y);
          doc
            .fillColor(lease.urgencyColor)
            .font('Helvetica-Bold')
            .text(`${lease.diffDays}d`, 335, y, { width: 55, align: 'center' });
          doc
            .fillColor('#334155')
            .font('Helvetica')
            .text(`LKR ${(lease.monthlyRent || 0).toLocaleString()}`, 395, y, {
              width: 90,
              align: 'right',
            });
          doc
            .fillColor(lease.urgencyColor)
            .font('Helvetica-Bold')
            .text(urgencyLabel, 490, y, { width: 60, align: 'center' });
          doc.font('Helvetica').fillColor('black');
          y += 18;
        }
        y += 10;
        ...
        if (critical.length > 0) {
          const criticalRent = critical.reduce(
          ...
          y = drawRecommendation(
            doc,
            y,
            `${critical.length} lease${critical.length > 1 ? 's expire' : ' expires'} within 14 days, representing LKR ${criticalRent.toLocaleString()}/month in revenue. Immediately contact these tenants to negotiate renewals or begin vacancy listing.`,
            'critical'
          );
        }
        if (urgent.length > 0) {
          ...
          y = drawRecommendation(
            doc,
            y,
            `${urgent.length} lease${urgent.length > 1 ? 's expire' : ' expires'} within 15–30 days. Send formal renewal notices and schedule tenant meetings this week.`,
            'warning'
          );
        }
        if (
          ...
          y = drawRecommendation(
            doc,
            y,
            `${upcoming.length} lease${upcoming.length > 1 ? 's' : ''} expiring in 31–90 days. Begin proactive renewal conversations to ensure continuity and avoid last-minute vacancies.`,
            'info'
          );
        }
      }
  ...
  async generateLeadConversionReport(req, res) {
    try {
      ...
      res.setHeader(
        'Content-Disposition',
        `inline; filename=lead_conversion_report.pdf`
      );
      doc.pipe(res);
      ...
      drawReportHeader(
        ...
        'Pipeline performance, conversion efficiency, and lead management insights'
      );
      doc.moveDown(0.5);
      ...
      const funnelSteps = [
        ...
        {
          label: 'Interested (Active Pipeline)',
          count: stats.Interested,
          color: '#60a5fa',
          rate:
            stats.Total > 0
              ? ((stats.Interested / stats.Total) * 100).toFixed(0)
              : '0',
        },
        {
          ...
          rate:
            stats.Total > 0
              ? ((stats.Converted / stats.Total) * 100).toFixed(0)
              : '0',
        },
        {
          label: 'Dropped / Lost',
          ...
          rate:
            stats.Total > 0
              ? ((stats.Dropped / stats.Total) * 100).toFixed(0)
              : '0',
        },
      ];
      ...
      funnelSteps.forEach((step, index) => {
        ...
        doc
          ...
          .font('Helvetica-Bold')
          .fontSize(12)
          .text(`${step.count}`, x, y + 4, { width, align: 'center' });
        doc
          .fillColor('white')
          .font('Helvetica')
          .fontSize(8)
          .text(step.label, x, y + 20, { width, align: 'center' });
        doc.restore();
        ...
        if (step.rate !== null) {
          doc
            ...
            .font('Helvetica')
            .fontSize(9)
            .text(`${step.rate}% of total`, x + width + 15, y + 12);
        }
        doc.fillColor('black');
        y += 48;
      });
      ...
      if (stats.Total === 0) {
        y = drawRecommendation(
          doc,
          y,
          'No leads have been captured yet. Start marketing your properties and add incoming inquiries as leads to track conversion.',
          'warning'
        );
      } else {
        if (Number(convRate) >= 30) {
          y = drawRecommendation(
            doc,
            y,
            `Strong conversion rate at ${convRate}%. Your lead-to-tenant pipeline is performing well. Continue current engagement strategies.`,
            'success'
          );
          ...
          y = drawRecommendation(
            doc,
            y,
            `Conversion rate of ${convRate}% is moderate. Review follow-up timing and property presentation. Consider scheduling site visits sooner after initial inquiry.`,
            'warning'
          );
        } else {
          y = drawRecommendation(
            doc,
            y,
            `Conversion rate of ${convRate}% is low. Review your lead engagement process — are follow-ups timely? Are leads being qualified properly? Consider improving property listings and response times.`,
            'critical'
          );
        }
        ...
        if (Number(dropRate) > 40) {
          ...
          y = drawRecommendation(
            doc,
            y,
            `Drop-off rate is high at ${dropRate}%. Investigate why leads are being lost — common causes include slow follow-up, uncompetitive pricing, or poor first impressions during site visits.`,
            'critical'
          );
        }
        ...
        if (activeLeads > 0) {
          ...
          y = drawRecommendation(
            doc,
            y,
            `${activeLeads} lead${activeLeads > 1 ? 's are' : ' is'} currently in the active pipeline. Ensure timely follow-ups to maximize conversions and prevent drop-offs.`,
            'info'
          );
        }
      }
