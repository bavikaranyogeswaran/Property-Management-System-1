...
class RenewalRequestModel {
    async create(data, connection = null) {
        const { leaseId, currentMonthlyRent, proposedMonthlyRent, proposedEndDate, status, notes } = data;
        const conn = connection || pool;
        const [result] = await conn.query(
            `INSERT INTO renewal_requests 
             (lease_id, current_monthly_rent, proposed_monthly_rent, proposed_end_date, status, negotiation_notes)
             VALUES (?, ?, ?, ?, ?, ?)`,
            [leaseId, currentMonthlyRent, proposedMonthlyRent || null, proposedEndDate || null, status || 'pending', notes || null]
        );
    ...
    async updateStatus(id, status, connection = null) {
        const conn = connection || pool;
        await conn.query(`UPDATE renewal_requests SET status = ? WHERE request_id = ?`, [status, id]);
    }
